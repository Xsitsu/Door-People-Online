...
void PhysicsSettings::Encode(void *&data) const
{
    ...
    PacketBuilder::Put32(data, static_cast<uint32_t>(this->settings.GetGravity()));
    PacketBuilder::Put32(data, static_cast<uint32_t>(this->settings.GetDefaultWalkspeed()));
    PacketBuilder::Put32(data, static_cast<uint32_t>(this->settings.GetDefaultJumpPower()));
}
...
void PhysicsSettings::Decode(unsigned int packet_size, void *&data)
{
    ...
    this->settings.gravity = static_cast<int>(PacketReader::Read32(data));
    this->settings.default_walkspeed = static_cast<int>(PacketReader::Read32(data));
    this->settings.default_jump_power = static_cast<int>(PacketReader::Read32(data));
}
