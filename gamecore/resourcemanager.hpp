...
public:
    ...
    bool ResourceExists(const res_handle &handle) const;
    int MaxNumResources() const;
    int CurNumResources() const;

private:
    bool HandleIsInRange(const res_handle &handle) const;
...
template <typename T>
bool ResourceManager<T>::HandleIsInRange(const res_handle &handle) const
{
